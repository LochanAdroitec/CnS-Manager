const API_BASE_URL = '/ViewerDocuments';
let pdfDoc = null;
let pageNum = 1;
...
async function loadBookmarkStatus() {
    try {
        const response = await fetch(`${API_BASE_URL}/ViewerBookmarks/${documentId}`);
        if (response.ok) {
...
async function saveBookmark() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/ViewerBookmarks/${documentId}?page=${pageNum}&name=${encodeURIComponent(bookmarkName)}`, {
            method: 'POST'
        });
...
async function deleteBookmarkById(bookmarkId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/ViewerBookmarks/delete/${bookmarkId}`, {
            method: 'DELETE'
        });
...
async function loadDocument(id, startPage = 1) {
    try {
        ...
        const pdfResponse = await fetch(`${API_BASE_URL}/viewer-view/${id}`);
